...
void CRemoteCode::PushParameter(parameter_type_t param_type, void *param, int rspOffset)
{
	parameter_info_t pi;

	pi.ptype = param_type;
	pi.pparam = param;
	pi.stackOffset = rspOffset;

	DbgShout("Adding parameter to function [%i][0x%IX]", pi.ptype, pi.pparam);

	m_CurrentInvokeInfo.params.push_back(pi);
}

void CRemoteCode::MoveOntoStack( __int64 val, int rspOffset )
{
	__int64 *pVal = new __int64;
	*pVal = val;
	PushParameter( PARAM_TYPE_INT64, pVal, rspOffset );
}
...
void CRemoteCode::LoadStringParam64(parameter_info_t paraminfo, parameter_index_t paramindex)
{
	if (paraminfo.ptype == PARAM_TYPE_STRING)
	{
		...
		if (s.ptr == NULL)
		{
			DbgShout("NULL Allocated ANSI string pointer....");
			return;
		}
		...
		if (m_bIs64bit)
			...
			DbgShout("[LoadStringType64] Not a 64 bit process!");
	}
	else if (paraminfo.ptype == PARAM_TYPE_WSTRING)
	{
		...
		if (s.ptr == NULL)
		{
			DbgShout("NULL Allocated UNICODE string pointer....");
			return;
		}
...
{
	DbgShout("PushCall [%s][0x%IX]", CallingConventionToString(cconv).c_str(), CallAddress);

	// Number of params
	int iFunctionBegin = (int)m_CurrentInvokeInfo.params.size();
	// Call address
	m_CurrentInvokeInfo.calladdress = (size_t)CallAddress;
	// Calling convention
	m_CurrentInvokeInfo.cconv = cconv;

	if ((m_bIs64bit || cconv == CCONV_WIN64) || cconv == CCONV_FASTCALL)
	{
		DbgShout("Entering __fastcall");
		...
		if (m_bIs64bit) // 64 bit
		{
			...
			rsp_dif = Utils::Align(rsp_dif, 0x10); // 16 byte aligned
			// sub  rsp, rsp_dif
			AddByteToBuffer(0x48);
			AddByteToBuffer(0x83);
			AddByteToBuffer(0xEC);
			AddByteToBuffer((unsigned char)rsp_dif + sizeof(size_t));
			...
			// Push the remainder of parameters on the stack
		...
		{
			if (iFunctionBegin == 0)  // is actually a stdcall
			{
				PushCall(CCONV_STDCALL, CallAddress);
				return;
			}
			else if (iFunctionBegin == 1) // is actually a stdcall
			{
			...
			{
				unsigned long ulEcxParam = *(unsigned long *)m_CurrentInvokeInfo.params[0].pparam; // edx param
				unsigned long ulEdxParam = *(unsigned long *)m_CurrentInvokeInfo.params[1].pparam; // eax param
				// mov ecx, ulEcxParam
				AddByteToBuffer(0xB9);
				AddLongToBuffer(ulEcxParam);
				// mov edx, ulEdxParam
				AddByteToBuffer(0xBA);
				AddLongToBuffer(ulEdxParam);

				m_CurrentInvokeInfo.params.erase(m_CurrentInvokeInfo.params.begin()); // erase ecx (first) param
				m_CurrentInvokeInfo.params.erase(m_CurrentInvokeInfo.params.begin()); // erase edx (second) param
	...
	{
		DbgShout("Entering __thiscall");

		if (iFunctionBegin == 0) { //no params...
			DbgShout("No parameters passed for __thiscall, requires at least one parameter (ECX)");
		}
		...
		if (m_CurrentInvokeInfo.params[0].ptype != PARAM_TYPE_POINTER) {
			DbgShout("Warning: \"THIS\" parameter type invalid [%i], should be PARAM_TYPE_POINTER", m_CurrentInvokeInfo.params[0].ptype);
		}

		void *pThis = m_CurrentInvokeInfo.params[0].pparam;
		if (pThis == NULL) {
			DbgShout("Warning: \"THIS\" parameter NULL for __thiscall function (ECX)");
		}
...
bool CRemoteCode::ExecuteRemoteThreadBuffer(remote_thread_buffer_t thread_data, bool async)
{
	...
	if ( hThreadHandle == INVALID_HANDLE_VALUE )
	{
		...
		DbgShout("Failed to execute remote buffer in process 0x%X", m_hProcess);
		
		return false;
	}

	DbgShout("Remote Buffer Executed in process 0x%X", m_hProcess);
...
void CRemoteCode::EndCallNoRet64( )
{
...
{
	//
	//DbgShout("Byte added to buffer: 0x%.2X", in);
...
void CRemoteCode::PushAllParameters(bool right_to_left)
{
	...
	DbgShout("Number of parameters for function [%i]", m_CurrentInvokeInfo.params.size());
	...
	if (right_to_left == false)
	{
		...
		for (int i = 0; i < (int)m_CurrentInvokeInfo.params.size(); i++)
		{
			...
			DbgShout("Parameter found [%i][%s]", i, ParameterTypeToString(m_CurrentInvokeInfo.params.at(i).ptype).c_str());
		}
	}
	else
	{
		...
		if (m_CurrentInvokeInfo.params.size() == 1)
		{
			...
			DbgShout("Parameter found [%i][%s]", 0, ParameterTypeToString(m_CurrentInvokeInfo.params.at(0).ptype).c_str());
		}
		else
		{
			...
			while (iBegin != -1)
			{
				...
				DbgShout("Parameter found [%i][%s]", iBegin, ParameterTypeToString(m_CurrentInvokeInfo.params.at(iBegin).ptype).c_str());
				iBegin--;
			}
		}
	}
	...
	for (int p = 0; p < (int)pushOrder.size(); p++)
	{
		...
		DbgShout("Function Iter [%i] Parameter [%s]", p, ParameterTypeToString(paraminfo->ptype).c_str());
		...
		case PARAM_TYPE_INT64:		//
		{
			if (paraminfo->pparam)
			{
				if (m_bIs64bit)
				{
					...
					AddLong64ToBuffer( ulParam ); // calladdress

					if ( paraminfo->stackOffset > 0 )
					{
						// mov [rsp + stackOffset], r13
						AddByteToBuffer( 0x4C );
						AddByteToBuffer( 0x89 );
						AddByteToBuffer( 0xAC );
						AddByteToBuffer( 0x24 );
						AddLongToBuffer( (unsigned long)paraminfo->stackOffset );
					}
					else
					{  
						//push r13	
		...
		case PARAM_TYPE_POINTER:	// 8 or 4 bytes, depending on architecture
		{
			if (paraminfo->pparam)
			{
				size_t ulParam = *(size_t*)paraminfo->pparam;
		...
		case PARAM_TYPE_UNICODE_STRUCT:
		{
			...
			struct_alloc_t unicodeStringAlloc;
			unicodeStringAlloc.size = sizeof(UNICODE_STRING);
			unicodeStringAlloc.ptr = CommitMemory(&unicodeParamAlloc, sizeof(UNICODE_STRING));
			m_CurrentInvokeInfo.structs.push_back(unicodeStringAlloc);

			if ( m_bIs64bit )
			{
				// mov r13, unicodeStringAlloc.ptr
				AddByteToBuffer( 0x49 );
				AddByteToBuffer( 0xBD );
				AddLong64ToBuffer( (unsigned __int64)unicodeStringAlloc.ptr );
				// push r13
				AddByteToBuffer( 0x41 );
				AddByteToBuffer( 0x55 );
			}
		...
		default:
		{
			DbgShout("Unable to locate parameter type %i", paraminfo->ptype);
			break;
		}
...
DWORD CRemoteCode::CreateRPCEnvironment(bool noThread /*= false*/)
{
	...
	if (thdID == 0 || status == false)
	{
		...
		DbgShout("[CreateRPCEnvironment] Error: %s", Utils::GetLastErrorAsString().c_str());
	}
	...
	if (!m_hWorkThd)
	{
		...
		*/
		...
		if (bWrite == FALSE)
		{
			...
			DbgShout("[CreateWorkerThread] Failed to allocate m_pWorkerCode!");
		...
		DbgShout("[CreateWorkerThread] Worker thread ID: %d ", GetThreadId(m_hWorkThd));
...
{         
    if(!m_hWaitEvent)
    {
		...
		if (pCodecave == NULL)
		{
			...
			DbgShout("[CreateAPCEvent] Failed to allocate pCodecave!");
		...
		if (hThread)
		{
			...
			if (GetExitCodeThread(hThread, (LPDWORD)&dwResult) == 0) {
				DbgShout("[CreateAPCEvent] Failed to get hThread Exit Code! Error: %s", Utils::GetLastErrorAsString().c_str());
			}
		}
