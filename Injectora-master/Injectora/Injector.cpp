...
BOOL Injector::LoadLibraryInject(String filePath)
{
	...
	HMODULE returnedModule = remoteLoader.LdrpLoadDllA( filePath.getCharPointer( ) );
	if (returnedModule)
	{
	...
	{
		char output[1024];
		sprintf_s(output, "LdrpLoadDll Failed: %s", Utils::GetLastErrorAsString().c_str());
		MessageBox(0, output, "Injectora", MB_ICONERROR);
		bRet = FALSE;
	}
