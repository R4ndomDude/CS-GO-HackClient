...
namespace Utils
{
	...
	static HMODULE GetLocalModuleHandle(const char* moduleName, unsigned int* moduleSize = NULL);
	...
	static void CreateDebugConsole(LPCSTR lPConsoleTitle)
	{
		...
		errno_t ret = freopen_s(&stream, "CONOUT$", "w", stdout);
		if (ret != NULL)
			return;
	...
	static HMODULE GetLocalModuleHandle(const char* moduleName, unsigned int* moduleSize)
	{
		void* dwModuleHandle = 0;
	...
	static int GetProcessPlatform(HANDLE hProcess)
	{
		...
		{
			#if defined(_M_IX86)
			return ProcessPlatformX86; // ProcessPlatformX86;
			#elif defined(_M_X64)
			return ProcessPlatformX64; // ProcessPlatformX64
			#endif
		}
	...
	__inline bool CompareData( unsigned char* data, unsigned char* byteMask, char* mask )
	{
		for ( ; *mask; ++mask, ++data, ++byteMask )
			if ( *mask == 'x' && *data != *byteMask )
				return false;
		return true;
	}

	static size_t FindPattern( HANDLE hProcess, size_t address, unsigned int size, unsigned char* byteMask, int n )
	{
		char mask[128];
		for ( int i = 0; i <= n; i++ ) {
			if ( i == n )
				mask[i] = '\0';
			else if ( byteMask[i] == 0xCC )
				mask[i] = '?';
			else
				mask[i] = 'x';
		}
		...
		SIZE_T size_return;
		if ( !ReadProcessMemory( hProcess, (void*)address, data, size, &size_return ) ) {
			delete[] data;
			return 0;
		}

		for ( size_t i = 0; i < size; ++i ) {
			if ( CompareData( (unsigned char*)(data + i), byteMask, mask ) ) {
				delete[] data;
				return (size_t)((size_t)address + i);
			}
		}
